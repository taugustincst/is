...
(async () => {
  ...
  const shape = await page.evaluate(() => ({ chapters: CAMPAIGN.length, acts: ACTS.map(a => [a.from, a.to]), last: CAMPAIGN[CAMPAIGN.length - 1].id, end: CAMPAIGN[CAMPAIGN.length - 1].outro.join(' ').includes('THE END.'), sea: CAMPAIGN.slice(17).map(c => MAPS[c.map].mood), finals: CAMPAIGN.filter(c => c.final).map(c => c.id), route: WORLD_ROUTE.length, cities: CITIES.length, water: !!ELEMENTS.water && !!ELEMENT_FX.water && typeof FX_DRAW.wave === 'function' && !!TRACKS.tide && !!TRACKS.deep }));
  ok('the road runs twenty-two chapters in four acts and ends at the Tidestone', shape.chapters === 22 && shape.acts.length === 4 && shape.acts[3][0] === 17 && shape.acts[3][1] === 21 && shape.last === 'ch22' && shape.end && shape.finals.join() === 'ch22' && shape.route === 22 && shape.cities === 8 && shape.sea.every(m => ['tide', 'storm', 'abyss'].includes(m)) && shape.water, JSON.stringify(shape));
