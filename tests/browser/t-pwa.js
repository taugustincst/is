...
(async () => {
  ...
  await page.evaluate(async () => { const c = await caches.open('elderon-v1'); for (const k of await c.keys()) if (k.url.endsWith('/js/game.js')) await c.delete(k); });
  await page.reload();
  await page.waitForFunction(async () => { const c = await caches.open('elderon-v1'); return (await c.keys()).some(k => k.url.endsWith('/js/game.js')); }, null, { timeout: 20000 }).catch(() => {});
  const refilled = await page.evaluate(async () => { const c = await caches.open('elderon-v1'); return (await c.keys()).some(k => k.url.endsWith('/js/game.js')); });
