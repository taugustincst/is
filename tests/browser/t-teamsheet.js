...
(async () => {
  ...
  const shot = await page.evaluate(() => {
    const jobs = ['squire','knight','archer','monk','thief','whiteMage','blackMage','ninja','dragoon','chemist','timeMage','darkKnight'];
    const kits = [
      { label: 'bare', gear: {} },
      { label: 'leather', gear: { weapon: 'broadsword', offhand: 'buckler', head: 'leatherCap', body: 'leatherArmor' } },
  ...
  require('fs').writeFileSync(`${S}/teamsheet.png`, Buffer.from(shot.split(',')[1], 'base64'));
