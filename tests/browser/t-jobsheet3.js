...
(async () => {
  ...
  const shot = await page.evaluate(() => {
    const jobs = ['paladin','arcanist','assassin','sage','samurai','bard'];
    const kits = [
      { label: 'bare', gear: {} },
      { label: 'greatsword', gear: { weapon: 'excalibur', body: 'crusaderMail', head: 'crusaderHelm' } }, { label: 'tome', gear: { weapon: 'omnibus', body: 'robeOfLords', head: 'sageCrown' } },
  ...
  require('fs').writeFileSync(`${S}/jobsheet3.png`, Buffer.from(shot.split(',')[1], 'base64'));
