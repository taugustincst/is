...
(async () => {
  ...
  const shot = await page.evaluate(() => {
    const jobs = ['samurai','summoner','geomancer','bard','knight','ninja'];
    const kits = [
      { label: 'bare', gear: {} },
      { label: 'own', gear: { weapon: 'kotetsu', body: 'leatherArmor' } }, { label: 'harp', gear: { weapon: 'faerieHarp', body: 'silkRobe' } },
  ...
  require('fs').writeFileSync(`${S}/jobsheet.png`, Buffer.from(shot.split(',')[1], 'base64'));
